...
本仓库当前提供和以后发布的版本均采用 [PolyForm Noncommercial 1.0.0](LICENSE)：仅非商业用途可以免费使用、修改和分发。版本号本身不授予 MIT 或其他商业使用许可。

任何直接或间接的商业使用，包括企业内部使用、营利活动、收费产品或服务、客户交付、重新包装分发和商业集成，都必须事先取得单独的书面付费商业授权。具体范围见 [商业授权说明](COMMERCIAL_LICENSE.md)。

本项目源码公开，但因限制商业使用，不属于 OSI 定义的开源软件。项目名称、商标、品牌、图标和视觉素材不随源码许可证一并授权。

过去已经合法取得且当时明确附带 MIT License 的历史副本，仍按其当时条款处理；该历史许可不会延伸到当前或后续取得的副本。
